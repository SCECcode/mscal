/**
 * @file muscalnc.c
 * @brief Main file for muscalnc model
 * @version 1.0
 *
 * @section DESCRIPTION
 *
...
char *muscalnc_config_string=NULL;
int muscalnc_config_sz=0;

// Constants
/** The version of the model. */
const char *muscalnc_version_string = "muscalnc";
...
muscalnc_model_t *muscalnc_velocity_model;

/**
 * Initializes the muscalnc plugin model within the UCVM framework. In order to initialize
...
int muscalnc_init(const char *dir, const char *label) {
    ...
    if(muscalnc_ucvm_debug) {
      stderrfp = fopen("muscalnc_debug.log", "w+");
      fprintf(stderrfp,"\n===== START muscalnc ===== \n\n");
    }

    // Initialize variables.
    
    muscalnc_configuration = calloc(1, sizeof(muscalnc_configuration_t));
    muscalnc_config_string = calloc(MUSCALNC_CONFIG_MAX, sizeof(char));
    muscalnc_config_string[0]='\0';
    muscalnc_config_sz=0;

    muscalnc_velocity_model = calloc(1, sizeof(muscalnc_model_t));
    muscalnc_velocity_model_init(muscalnc_velocity_model);

    // Configuration file location.
    sprintf(configbuf, "%s/model/%s/data/config", dir, label);

    // Read the configuration file.
    if (muscalnc_read_configuration(configbuf, muscalnc_configuration) != UCVM_MODEL_CODE_SUCCESS) {
           // Try another, when is running in standalone mode..
       sprintf(configbuf, "%s/data/config", dir);
       if (muscalnc_read_configuration(configbuf, muscalnc_configuration) != UCVM_MODEL_CODE_SUCCESS) {
           muscalnc_print_error("No configuration file was found to read from.");
    ...
    muscalnc_velocity_model->dataset_cnt=muscalnc_configuration->dataset_cnt;
    tempVal = muscalnc_read_model(muscalnc_configuration, muscalnc_velocity_model, muscalnc_data_directory);
    ...
    if (tempVal == SUCCESS) {
      if(muscalnc_ucvm_debug) {
        ...
        muscalnc_print_error("No model file was found to read from.");
        return FAIL;
    }

    // setup config_string 
    sprintf(muscalnc_config_string,"config = %s\n",configbuf);
    muscalnc_config_sz=1;

    // Let everyone know that we are initialized and ready for business.
    muscalnc_is_initialized = 1;

    return SUCCESS;
}

/**
 * Queries muscalnc at the given points and returns the data that it finds.
 *
 ...
 */
int muscalnc_query(muscalnc_point_t *points, muscalnc_properties_t *data, int numpoints) {

if(muscalnc_ucvm_debug){ fprintf(stderrfp,"\ncalling muscalnc_query with %d numpoints\n",numpoints); }
...
//  hold coord point's info
    muscalnc_pt_info_t *pt_info = (muscalnc_pt_info_t  *) malloc(numpoints * sizeof(muscalnc_pt_info_t));
...
// iterate through all the points and compose the buffers for all index
	...
	if(muscalnc_configuration->interpolation) { // fill cell percent
...
// retrieve and disperse the result back into data
    ...
    if(!muscalnc_configuration->too_big) { 

if(muscalnc_ucvm_debug){ fprintf(stderrfp,">> Using In-Memory access \n"); }

        // should be in the in-memory 
        for(int i=0; i<numpoints; i++) {
            if(!muscalnc_configuration->interpolation) { 
		// no interp
	    ...
	    // If result is None, then need to process for muscalnc1d
	    // with muscalnc1d with depth
            if(isnan(data[i].vp) && isnan(data[i].vs) && muscalnc_configuration->enable_1d) {

if(muscalnc_ucvm_debug){ fprintf(stderrfp,">> calling get_one_nuscal1d_property\n"); }
                get_one_muscalnc1d_property(dataset, &(pt_info[i]), &(data[i]));
            }
        }
...
//
// WARNING : no interpolation is done, for MUSCALNC, THIS SHOULD NOT BE USED 
// EXCEPT FOR TESTING
//
...
//    and then extract data from buffer one at a time using dep_idx 
        if(same_lon_idx && same_lat_idx ) {
          // grab a col from cache
            muscalnc_cache_col_t *col=find_a_cache_col(dataset, first_lat_idx, first_lon_idx); 
if(muscalnc_ucvm_debug){ fprintf(stderrfp,">> Using External data/Col cache - col idx=%d\n", dataset->col_cache_cnt); }
...
// and then extract data from buffer using lon_idx, and lat_idx
          ...
          // grab a layer from cache or try to load it from external data file
            muscalnc_cache_layer_t *layer=find_a_cache_layer(dataset, first_dep_idx);

if(muscalnc_ucvm_debug){ fprintf(stderrfp,">> Using External data/Layer cache - current count=%d\n", dataset->layer_cache_cnt); }
...
if(muscalnc_ucvm_debug){ fprintf(stderrfp,">> Using External data/Random call \n"); }
            for(int i=0; i<numpoints; i++) { 
                lon_idx=pt_info[i].lon_idx;
                lat_idx=pt_info[i].lat_idx;
...
int muscalnc_finalize() {
    ...
    muscalnc_is_initialized = 0;

    if (muscalnc_configuration) {
        muscalnc_configuration_finalize(muscalnc_configuration);
    }
    if (muscalnc_velocity_model) {
        muscalnc_velocity_model_finalize(muscalnc_velocity_model);
    }
...
{
  int verlen;
  verlen = strlen(muscalnc_version_string);
  if (verlen > len - 1) {
    verlen = len - 1;
  }
  memset(ver, 0, len);
  strncpy(ver, muscalnc_version_string, verlen);
  return 0;
}
 ...
 * @return Zero
 */
int muscalnc_config(char **config, int *sz)
{
  int len=strlen(muscalnc_config_string);
  if(len > 0) {
    *config=muscalnc_config_string;
    *sz=muscalnc_config_sz;
    return SUCCESS;
  }
  return FAIL;
}
...
/**
 * Reads the muscalnc_configuration file describing the various properties of MUSCALNC and populates
 * the muscalnc_configuration struct. This assumes muscalnc_configuration has been "calloc'ed" and validates
 * that each value is not zero at the end.
 *
 * @param file The muscalnc_configuration file location on disk to read.
 * @param config The muscalnc_configuration struct to which the data should be written.
...
int muscalnc_read_configuration(char *file, muscalnc_configuration_t *config) {
    ...
    // Read the lines in the muscalnc_configuration file.
    while (fgets(line_holder, sizeof(line_holder), fp) != NULL) {

        if (line_holder[0] != '#' && line_holder[0] != ' ' && line_holder[0] != '\n') {
            _splitline(line_holder, key, value);
            if(muscalnc_ucvm_debug){ fprintf(stderrfp, "a config line: %s\n", line_holder); }
...
if(muscalnc_ucvm_debug){ fprintf(stderrfp, "enabled Interpolation\n"); }
                }
            }
            if (strcmp(key, "binary_data") == 0) { 
                config->use_binary=0;
                if (strcmp(value,"on") == 0) { config->use_binary=1;
if(muscalnc_ucvm_debug){ fprintf(stderrfp, "enabled Binary data\n"); }
                }
            }
            if (strcmp(key, "too_big") == 0) { 
                config->too_big=0;
                if (strcmp(value,"on") == 0) { config->too_big=1;
if(muscalnc_ucvm_debug){ fprintf(stderrfp, "enabled tooBig -- use external data access\n"); }
                }
            }
            if (strcmp(key, "enable_1d") == 0) { 
                config->enable_1d=0;
                if (strcmp(value,"on") == 0) { config->enable_1d=1;
if(muscalnc_ucvm_debug){ fprintf(stderrfp, "enabled muscalnc1d to fill all empty returns\n"); }
                }
            }
         /* for each dataset, allocate a model dataset's block and fill in */ 
            if (strcmp(key, "data_file") == 0) { 
                if( config->dataset_cnt < MUSCALNC_DATASET_MAX) {
                  int rc=_setup_a_dataset(config,value);
                  if(rc == 1 ) { config->dataset_cnt++; }
                } else { muscalnc_print_error("Exceeded dataset maximum limit."); }
	    } 
        }
    }

    // Have we set up all muscalnc_configuration parameters?
    if (config->utm_zone == 0 || config->model_dir[0] == '\0' || config->dataset_cnt == 0 ) {
        muscalnc_print_error("One muscalnc_configuration parameter not specified. Please check your muscalnc_configuration file.");
        return UCVM_MODEL_CODE_ERROR;
    }
...
/**
 * Extract muscalnc netcdf dataset specific info
 * and fill in the model info, one dataset at a time
 * and allocate required memory space
 *
 * @param 
 */
int _setup_a_dataset(muscalnc_configuration_t *config, char *blobstr) {
    ...
    if(confjson == NULL) {
        ...
        if(eptr != NULL) {
            if(muscalnc_ucvm_debug){ fprintf(stderrfp, "Configuration dataset setup error: (%s)\n", eptr); }
            return FAIL;
        }
    }
 ...
 *
 * */
int muscalnc_read_model(muscalnc_configuration_t *config, muscalnc_model_t *model, char *datadir) {

    int max_idx=model->dataset_cnt; // how many datasets are there
    for(int i=0; i<max_idx;i++) { 
        muscalnc_dataset_t *data=make_a_muscalnc_dataset(datadir, config->dataset_files[i], config->too_big, config->use_binary); 
...
int muscalnc_velocity_model_init(muscalnc_model_t *model) {
    model->dataset_cnt=0;
    for(int i=0; i<MUSCALNC_DATASET_MAX; i++) {
...
void muscalnc_print_error(char *err) {
    fprintf(stderr, "An error has occurred while executing muscalnc. The error was: \n\n%s\n",err);
    fprintf(stderr, "\nPlease contact software@scec.org and describe both the error and a bit\n");
    fprintf(stderr, "about the computer you are running muscalnc on (Linux, Mac, etc.).\n");
}
 ...
 *
 */
static int data_too_big(muscalnc_dataset_t *dataset) {
...
/**
 * Init function loaded and called by the UCVM library. Calls muscalnc_init.
 *
 * @param dir The directory in which UCVM is installed.
 * @return Success or failure.
 */
int model_init(const char *dir, const char *label) {
    return muscalnc_init(dir, label);
}

/**
 * Query function loaded and called by the UCVM library. Calls muscalnc_query.
 *
 ...
 * @return Success or fail.
 */
int model_query(muscalnc_point_t *points, muscalnc_properties_t *data, int numpoints) {
    return muscalnc_query(points, data, numpoints);
}

/**
 * Finalize function loaded and called by the UCVM library. Calls muscalnc_finalize.
 *
 * @return Success
 */
int model_finalize() {
    return muscalnc_finalize();
}

/**
 * Version function loaded and called by the UCVM library. Calls muscalnc_version.
 *
 * @param ver Version string to return.
 * @param len Maximum length of buffer.
 * @return Zero
...
int model_version(char *ver, int len) {
    return muscalnc_version(ver, len);
}

/**
 * Version function loaded and called by the UCVM library. Calls muscalnc_config.
 *
...
int model_config(char **config, int *sz) {
    return muscalnc_config(config, sz);
}


int (*get_model_init())(const char *, const char *) {
        return &muscalnc_init;
}
int (*get_model_query())(muscalnc_point_t *, muscalnc_properties_t *, int) {
         return &muscalnc_query;
}
int (*get_model_finalize())() {
         return &muscalnc_finalize;
}
int (*get_model_version())(char *, int) {
         return &muscalnc_version;
}
int (*get_model_config())(char **, int*) {
    return &muscalnc_config;
}
